...
class SASTOrchestrator:
    ...
    def __init__(self, project_root: str = None, config_base_path: str = None):
        ...
        self.config_base_path = Path(config_base_path) if config_base_path else self.project_root
    ...
    def _load_sast_tools_config(self) -> Dict[str, Any]:
        ...
        sast_config_file = self.main_config.get('sast', {}).get('config_file', 'config/sast_tools.yaml')
    ...
    def _get_files_to_analyze(self, directory_path: Path) -> List[Path]:
        ...
        if len(filtered_files) > max_files_total:
            print(f"📊 Limitando análisis a {max_files_total} archivos (de {len(filtered_files)})")
    ...
    def _run_tool(self, tool_name: str, filepath: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if tool_name == 'cppcheck':
                # cppcheck necesita un enfoque especial para XML
                with tempfile.NamedTemporaryFile(mode='w', suffix='.xml', delete=False) as tmp:
                    xml_file = tmp.name
                
                # Comando específico para cppcheck con salida XML
                ...
                if tool_name == 'semgrep':
                    ...
                    # Parseo genérico
                    return self._parse_generic_output(tool_name, result.stdout, filepath)
    ...
    def _parse_semgrep_output(self, output: str, filepath: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for finding in data.get('results', []):
                ...
                if severity_order.get(severity, 99) > severity_order.get(min_severity, 99):
                    continue
                ...
                issues.append({
                    ...
                    'confidence': finding.get('extra', {}).get('confidence', 'MEDIUM'),
    ...
    def _parse_cppcheck_output(self, output: str, filepath: str) -> List[Dict[str, Any]]:
	    """Parsea la salida de cppcheck (puede ser XML o texto)"""
	    issues = []
	    ...
	    if output.strip().startswith('<?xml'):
	        try:
	            ...
	            for error in root.findall('.//error'):
	                severity = error.get('severity', 'style').upper()
	            ...
	            # Si falla XML, intentar parsear como texto
	            pass
    ...
    def _parse_generic_output(self, tool_name: str, output: str, filepath: str) -> List[Dict[str, Any]]:
    ...
    def _update_stats(self, issue: Dict[str, Any]):
        ...
        self.stats['issues_by_severity'][severity] = self.stats['issues_by_severity'].get(severity, 0) + 1
        
        # Contar por herramienta
        self.stats['issues_by_tool'][tool] = self.stats['issues_by_tool'].get(tool, 0) + 1
    ...
    def generate_report(self, output_format: str = None, output_dir: str = None) -> str:
        ...
        if output_format is None:
            output_format = self.sast_config.get('reporting', {}).get('default_format', 'json')
        
        if output_dir is None:
            output_dir = self.sast_config.get('reporting', {}).get('output_dir', 'reports/sast')
    ...
    def _generate_json_report(self, output_dir: Path, timestamp: str) -> str:
        ...
        report_data = {
            'metadata': {
                ...
                'tdh_version': self.main_config.get('engine', {}).get('version', '0.1.0')
            },
            'statistics': self.stats,
            'issues': self.results,
            'configuration': {
                'tools_used': [name for name, info in self.available_tools.items() if info['available']],
    ...
    def _generate_console_report(self):
        ...
        critical_issues = [issue for issue in self.results 
...
def run_sast_analysis(project_path: str = ".", config_path: str = None) -> Dict[str, Any]:
    """
